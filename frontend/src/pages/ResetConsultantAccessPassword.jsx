...
const ResetConsultantAccessPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.post('/auth/reset-consultant-access-password', {
        token,
        newPassword: password,
      });
      if (res.data.success) {
        toast.success(res.data.message || 'Consultant access password updated');
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-xl">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Set a new password for unlocking consultant profiles in the admin portal.
        ...
        {!token ? (
          <div className="p-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-2xl text-red-800 dark:text-red-400 text-sm">
            ...
            <p>This reset link is invalid or incomplete. Request a new one from Consultants → View details.</p>
            <div className="mt-4">
              <Link to="/admin/consultants" className="font-bold text-blue-600 hover:text-blue-500">
                Back to Consultants &rarr;
              </Link>
            </div>
          </div>
        ) : (
...
export default ResetConsultantAccessPassword;
