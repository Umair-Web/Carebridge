...
const AdminHospitals = () => {
  ...
  const [newDoctor, setNewDoctor] = useState({ name: '', specialty: '', consultationFee: '', email: '' });
  ...
  const loadOverrides = useCallback(async (hospitalRef, unlockToken) => {
    if (!hospitalRef || !unlockToken) return;
    setOvLoading(true);
    try {
      const res = await api.get(`/admin/hospitals/${hospitalRef}/consultant-overrides`, {
        headers: { 'X-Profile-Unlock': unlockToken },
      });
      if (res.data.success) {
  ...
  useEffect(() => {
    if (selected?._id && selected?.profile?._id && profileUnlockToken) {
      loadOverrides(selected._id, profileUnlockToken);
    } else {
      setOvList([]);
      setOvDraft({});
      setOvSearch('');
    }
  }, [selected?._id, selected?.profile?._id, profileUnlockToken, loadOverrides]);
  ...
  useEffect(() => {
    ...
    if (!hospitalId || !profileUnlockToken) {
      setPatients([]);
      return;
    }
  ...
  }, [selected?.profile?._id, profileUnlockToken]);

  const requestView = (hospital) => {
    setPendingView(hospital);
    setUnlockPassword('');
  };
  ...
  const submitUnlock = async (e) => {
    ...
    setUnlocking(true);
    try {
      const res = await api.post(`/admin/hospitals/${pendingView._id}/verify-password`, {
  ...
  const loadAdminDoctors = async (hospitalId) => {
    if (!hospitalId || !profileUnlockToken) return;
    try {
  ...
  const addAdminDoctor = async () => {
    ...
    try {
      ...
      setNewDoctor({ name: '', specialty: '', consultationFee: '', email: '' });
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {filtered.length === 0 ? (
              <tr><td colSpan={2} className="px-5 py-12 text-center text-slate-500">No hospitals found.</td></tr>
            ) : filtered.map(h => {
            ...
            <form onSubmit={submitUnlock} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2">
                ...
                <button
                  type="submit"
                  disabled={unlocking}
                  className="px-4 py-2.5 rounded-xl text-sm font-bold bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-50 transition-colors"
                >
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 space-y-4">
            ...
            <div className="border-t pt-4">
                ...
                {adminDoctors.map((d) => (
                  <li key={d._id} className="flex justify-between items-center bg-slate-50 px-2 py-1 rounded">
                    <span>
                      ...
                      {d.email ? <span className="text-slate-400 text-xs ml-1">· {d.email}</span> : null}
                    </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                <input className="border rounded-lg px-2 py-1.5 text-xs" placeholder="Email" type="email" value={newDoctor.email} onChange={(e) => setNewDoctor({ ...newDoctor, email: e.target.value })} />
              </div>
